...
def getInfoToposMSE(pro=True):
    """
    Carga el json del HMI topo con ctc, por defecto entorno pro, si es neceario obtener datos de sustituye por :http://topo.rail.api.pre.elcano.operaciones.adif/msetopo/findAllToposInfo"
    """
...
def getInfoToposMSEWithoutCTC(pro=True):
    """
    Carga el json del HMI topo sin CTC por defecto pro, para datos pre:http://topo.rail.api.pre.elcano.operaciones.adif/msetopo/findAllToposInfoWithoutCtc
    """
...
def getCirculacionesPlanificadas(date):
    ...
    """
    key = os.getenv("user-key")
    HOSTPATH = "https://datos-mse.api.adif/mse/api/planif/day"
...
def getHistoricoMOW(
    ...
    """
    HOSTPATH = "https://datos-mse.api.adif/mse/api/historic/mov"
...
def getTopo(interlock,ctc):
    """
    Carga la topologia de IHM
    """

    key = os.getenv("user-key")
    url = "https://datos-mse.api.adif/mse/api/date/topology"
...
def getCatalogue(ctc: str):
    """
    Descarga el catálogo de un CTC y lo convierte en DataFrame.
    Formato CSV: Acrónimo;Elemento;TipoId
    """
    key = os.getenv("user-key")
    url = "https://datos-mse.api.adif/mse/api/date/catalog"
    ...
    catalogue_resource = data.get("catalogueResource", {})
    config_csv         = catalogue_resource.get("configFileCsv", "")

    # ── Parsear CSV  (formato: Acrónimo;Elemento;TipoId) ──────────────────────
    rows = []

    for line in config_csv.splitlines():
        line = line.strip()
        ...
        if len(parts) >= 3:
            rows.append({
                "CTC": catalogue_id.get("ctc", ctc),
                "Acrónimo": parts[0].strip(),
                "Elemento": parts[1].strip(),
                "TipoId": parts[2].strip(),
            })

    df_catalogue = pd.DataFrame(rows)

    print(f"✅ Catálogo {ctc} descargado | {len(df_catalogue)} elementos "
          f"| Acrónimos únicos: {df_catalogue['Acrónimo'].nunique()}")
...
def uploadSharepoint(nombre_archivo: str, contenido_archivo: bytes, carpeta: str):
    ...
    upload_url = (
        ...
        f"/drive/root:/00-CALIDAD DATO/_Análisis Calidad Datos MSE y MIE/"
        f"{carpeta}/{nombre_archivo}:/content"
    )
...
def descargarSharepoint(nombre_archivo: str, carpeta: str):
    """
    Descargar un archivo de SharePoint.
...
def descargarSharepoint_bytesio(nombre_archivo: str, carpeta: str):
    resultado = descargarSharepoint(nombre_archivo, carpeta)
    ...
    return {
        "success": True,
        "file": nombre_archivo,
        "stream": BytesIO(resultado["content"])
    }
