...
from src.api.APIs import getPlanificacionCirculacionesTecnicas, hacerPeticion, uploadSharepoint
...
CARPETA_SHAREPOINT = "00.Rotulación-Fiabilidad-Supresiones/Rotulación"
...
def generar_pdf(origenes: pd.DataFrame, df_merge: pd.DataFrame) -> str:
    ...
    nombre_archivo = f"Semana{semana}_{fecha_formateada}_rotulaciones_erroneas.pdf"

    # El PDF se construye directamente en memoria; no se guarda ningún
    # fichero en local, se sube exclusivamente vía uploadSharepoint.
...
def guardar_excel(origenes: pd.DataFrame, df_merge: pd.DataFrame, fecha_ayer: datetime) -> None:
    """
    Genera el Excel y lo sube exclusivamente a SharePoint mediante uploadSharepoint.
    guardarExcelMulti necesita una ruta de fichero real para escribir, así que se
    usa un fichero temporal solo como paso intermedio: se crea, se lee su
    contenido, se sube a SharePoint y se borra inmediatamente. No queda ningún
    archivo persistente en local.
    """
    semana = fecha_ayer.isocalendar()[1]
    today_str = fecha_ayer.strftime("%Y-%m-%d")
    nombre_archivo = f"Semana{semana}_{today_str}_rotulaciones_erroneas.xlsx"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        fname = Path(tmp_dir) / nombre_archivo
    ...
    resultado = uploadSharepoint(
        nombre_archivo=nombre_archivo,
        contenido_archivo=contenido_excel,
        carpeta=CARPETA_SHAREPOINT,
    )

    if not resultado.get("success"):
        raise RuntimeError(f"Error al subir el Excel a SharePoint: {resultado}")

    print(f"✅ Excel subido a SharePoint: {nombre_archivo}")
